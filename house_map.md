...
<script>
  ...
  const legend = L.control({position: 'bottomright'});
      ...
      fetch('assets/data/house_districts.geojson')
        .then(res => res.json())
        .then(data => {
          const geoJsonLayer = L.geoJSON(data, {
            ...
            onEachFeature: function (feature, layer) {
              ...
              let popupContent = `<strong>House District ${props.name || 'N/A'}</strong><br>`;
          ...
          houseMap.fitBounds(geoJsonLayer.getBounds());
          addLegend(houseMap);
        })
        .catch(err => console.error('Error loading house districts:', err));
    });
</script>
